...
const loginRegisterChoice = document.querySelectorAll(
  ".login__login-register-choice-text"
);

loginRegisterChoice[0].addEventListener("click", () => {
  mainWrapper.classList.remove("login__main-wrapper--shift");
  loginRegisterChoice[0].classList.add(
    "login__login-register-choice-text--active"
  );
  loginRegisterChoice[1].classList.remove(
    "login__login-register-choice-text--active"
  );
});
loginRegisterChoice[1].addEventListener("click", () => {
  mainWrapper.classList.add("login__main-wrapper--shift");
  loginRegisterChoice[0].classList.remove(
    "login__login-register-choice-text--active"
  );
  loginRegisterChoice[1].classList.add(
    "login__login-register-choice-text--active"
  );
});
